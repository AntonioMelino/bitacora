...
- Budget: presupuesto planeado del viaje (decimal, opcional)
...
**Paso 7 · `feature/places-autocomplete`** *(mediano)* ✅ HECHO (2026-08-06)
Implementado con una alternativa gratuita en vez de Google Places API (sin API key, sin costo):
- Autocompletado vía la API Photon (basada en OpenStreetMap) al agregar una ciudad o un lugar para visitar
- Las sugerencias de lugares se acotan a la ciudad en la que se están agregando (se geocodifica el nombre de la ciudad una vez, y los resultados se restringen a un radio de ~30km), así por ejemplo "Coliseo" dentro de "Roma" solo devuelve resultados cerca de Roma
- Al elegir una sugerencia se completa automáticamente un link de búsqueda de Google Maps armado con sus coordenadas; los lugares sin link guardado (escritos a mano, o agregados antes de esta feature) siempre muestran un link de respaldo "Ver en Maps" generado a partir del nombre
...
**Paso 9 · `feature/testing-foundation`** *(grande)* ✅ HECHO (2026-08-14)
- Antes de este paso no existía ni un solo test en el repo
...
**Paso 10 · `feature/ci-pipeline`** *(pequeño)* ✅ HECHO (2026-08-11)
...
**Paso 12 · `feature/refresh-token`** *(pequeño)* ✅ HECHO (2026-08-13)
- Nueva entidad/tabla `RefreshToken` (migración `AddRefreshTokens`). `POST /api/auth/refresh` rota el token (de un solo uso — cada refresh emite uno nuevo y revoca el anterior), `POST /api/auth/logout` lo revoca del lado del servidor. `login`/`register` ahora también devuelven un `refreshToken` junto al JWT
- Frontend: nueva instancia axios compartida (`frontend/src/services/api.ts`) con un interceptor de request que agrega el header de autenticación y uno de response que refresca en silencio ante un 401 y reintenta la petición una vez (los 401 concurrentes comparten un único refresh en curso para no competir contra la rotación de un solo uso). Los 9 archivos de servicio de la API fueron migrados a esta instancia compartida, eliminando sus helpers duplicados `authHeaders()`/`h()`. `authService` ganó `logout()`, usado por el botón de cerrar sesión del dashboard

**Paso 13 · `feature/rate-limiting`** *(pequeño)* ✅ HECHO (2026-08-10)
- Middleware de rate limiting integrado de ASP.NET Core (`Microsoft.AspNetCore.RateLimiting`, sin paquete nuevo — es parte del framework), un limitador de ventana fija (`auth`: 5 peticiones por minuto por cliente, sin cola) aplicado con `[EnableRateLimiting("auth")]` en `AuthController.Register` y `AuthController.Login`
- Exceder el límite devuelve HTTP 429; no requiere cambios en el frontend porque axios ya trata las respuestas no-2xx como errores
...
**Paso 18 · `feature/trip-map-view`** *(mediano)* ✅ HECHO (2026-08-09)
...
**Paso 20 · `feature/soft-delete-undo`** *(pequeño)* ✅ HECHO (2026-08-13)
...
| 2026-07-02 | feature/password-visibility-toggle | Se agregó un ícono de ojito para mostrar/ocultar la contraseña en todos los campos de contraseña. Nuevo componente reutilizable `PasswordInput` (`frontend/src/components/ui/PasswordInput.tsx`) que alterna el input entre `type="password"` y `type="text"`, usado en LoginPage y RegisterPage (tanto en contraseña como en repetir contraseña). |
| 2026-07-03 | feature/date-picker-same-month | Los selectores de fecha en pareja ahora quedan posicionados en el mismo mes/año: nueva utilidad `alignEndDate()` (`frontend/src/utils/dates.ts`) que copia la fecha de inicio a la fecha de fin cuando esta está vacía o ya no es válida. Aplicado a la fecha de inicio/fin de viaje (NewTripModal y EditTripModal en DashboardPage.tsx/TripDetailPage.tsx) y al check-in/check-out de alojamientos (AccommodationsTab.tsx), así al abrir el segundo selector de fecha ya no salta al mes actual. |
| 2026-07-20 | feature/pwa-icons | Se registró el set de íconos de la app en `frontend/public/icons/`: nuevos `android-chrome-192x192.png`, `android-chrome-512x512.png`, `favicon-16x16.png`, `favicon-32x32.png`, y un `apple-touch-icon.png` actualizado. `frontend/index.html` ahora enlaza `favicon-32x32.png`/`favicon-16x16.png` como ícono de pestaña; se quitó el `<link>` de `favicon.svg` para que el nuevo PNG se muestre realmente (los navegadores priorizan SVG sobre PNG cuando ambos están declarados). El manifest de instalación de `vite-plugin-pwa` (`icon-192.png`, `icon-512.png`, `icon-maskable-512.png`) se dejó sin tocar para no generar un manifest duplicado/en competencia. |
| 2026-07-20 | feature/fix-icon-padding | Se corrigió un borde blanco visible alrededor del apple-touch-icon en la pantalla de inicio de iOS: los archivos de ícono fuente tenían un margen blanco incorporado alrededor del gráfico (5-8% del lienzo). Se recortó ese margen y se reescaló el contenido para que llegue hasta el borde en `apple-touch-icon.png`, `android-chrome-192x192.png`, `android-chrome-512x512.png`, `favicon-32x32.png` y `favicon-16x16.png`. |
| 2026-07-20 | feature/title-icon | Se agregó el ícono de la app al lado del título "Bitácora ✈️" en cada lugar donde aparece como logo estilizado. Nuevo componente compartido `AppTitle` (`frontend/src/components/ui/AppTitle.tsx`, con prop `size` para los dos tamaños de fuente en uso) que muestra `favicon-32x32.png` junto al texto. Reemplaza el `<span>` de título duplicado en `LandingPage.tsx` (nav), `LoginPage.tsx`/`RegisterPage.tsx` (logo arriba del formulario) y `DashboardPage.tsx` (header). Las menciones sueltas de "Bitácora" en texto corrido (footer, install prompt, copy de la landing) se dejaron sin tocar porque no son el título estilizado. |
| 2026-07-20 | feature/remove-unused-icons | Se eliminaron los íconos viejos reemplazados por el nuevo set: `favicon.svg`, `icons.svg`, `icon-192.png`, `icon-512.png`, `icon-maskable-512.png` borrados de `frontend/public/`. Se actualizó `vite.config.ts` (`vite-plugin-pwa`) para que `includeAssets` y el array `icons` del manifest de instalación ya no referencien los archivos borrados — el manifest ahora apunta a `android-chrome-192x192.png`/`android-chrome-512x512.png` (ya no hay un ícono maskable dedicado, así que se sacó la entrada con `purpose: 'maskable'` para no arriesgar un recorte feo en el ícono adaptativo de Android). |
...
| 2026-08-09 | feature/trip-map-view | Se agregó una sección "🧭 Mapa" al menú del viaje (`MapTab.tsx`, nueva entrada `map` en la lista de tabs de `TripDetailPage.tsx`) que muestra un mapa interactivo de Leaflet (tiles de OpenStreetMap, sin API key) con pines de cada ciudad (geocodificada al vuelo con el helper `geocodeCity` ya existente) y de cada lugar a visitar que tenga datos de coordenadas — las coordenadas se parsean del lado del cliente a partir del `mapsLink` ya guardado en `PlaceToVisit` (el formato `query=lat,lon` que genera `buildMapsLink`; los lugares que solo tienen un link de respaldo por nombre se omiten porque no hay coordenadas para ellos). Tocar un pin abre un popup con el nombre y un link "Ver en Google Maps". Se agregaron dependencias npm nuevas, mencionadas al usuario antes de instalarlas: `leaflet`, `react-leaflet`, `@types/leaflet` (dev). Sin cambios en el backend ni en la base de datos — City/PlaceToVisit siguen sin persistir su propia lat/lng. |
| 2026-08-10 | feature/rate-limiting | Se agregó rate limiting a los endpoints de autenticación usando el middleware integrado de ASP.NET Core `Microsoft.AspNetCore.RateLimiting` (sin paquete nuevo). `Program.cs` registra un limitador de ventana fija llamado `auth` (5 peticiones por minuto por cliente, sin cola, HTTP 429 al rechazar); `AuthController.Register` y `AuthController.Login` quedan anotados con `[EnableRateLimiting("auth")]`. Protege contra intentos de login por fuerza bruta y registros masivos falsos. No requirió cambios en el frontend. |
| 2026-08-11 | feature/ci-pipeline | Se agregó `.github/workflows/ci.yml`: un workflow de GitHub Actions que corre en cada push/PR a `main` o `develop`, con dos jobs en paralelo — `backend-build` (`dotnet restore` + `dotnet build --configuration Release` en `backend/`) y `frontend-build` (`npm ci` + `npm run build` en `frontend/`). Si cualquiera de los dos falla, el PR muestra una ❌ roja, dando visibilidad de builds rotos antes de mergear (no se configuró branch protection para bloquear el merge de forma dura — eso es un ajuste del repo de GitHub aparte, no del archivo de workflow). Sin paso de tests todavía — se agregará cuando llegue el Paso 9 (testing-foundation). Se verificó que ambos builds pasan localmente antes de commitear. |
| 2026-08-13 | feature/refresh-token | Flujo de refresh token para que un JWT vencido ya no rompa la sesión en silencio. Backend: nueva entidad `RefreshToken` + tabla `RefreshTokens` (migración `AddRefreshTokens`), `POST /api/auth/refresh` rota el refresh token (de un solo uso — emitir uno nuevo revoca el anterior), `POST /api/auth/logout` lo revoca del lado del servidor, `login`/`register` ahora también devuelven un `refreshToken`. Verificado manualmente de punta a punta contra la API local: registro → refresh (el token viejo se rechaza al reutilizarlo) → logout → refresh rechazado después; el rate limiter `auth` (Paso 13) también aplica correctamente a los endpoints nuevos. Frontend: nueva instancia axios compartida `frontend/src/services/api.ts` con un interceptor de request que agrega el header de autenticación y uno de response que refresca en silencio ante un 401 y reintenta una vez (los 401 concurrentes comparten un único refresh en curso para no competir contra la rotación de un solo uso); los 9 archivos de servicio de la API (`tripService`, `expenseService`, `itineraryService`, `checklistService`, `accommodationService`, `cityService`, `lookupService`, `simOptionService`, `exportService`) fueron migrados a esta instancia compartida, eliminando sus helpers duplicados `authHeaders()`/`h()`; `authService` ganó `logout()`, conectado al botón de cerrar sesión del dashboard. |
| 2026-08-13 | feature/soft-delete-undo | Al borrar un gasto, ítem de checklist, alojamiento, ciudad, lugar, día de itinerario u opción SIM, el ítem ahora se saca de la lista al instante pero la llamada real a la API se retrasa 5 segundos detrás de un toast "Deshacer" en la parte inferior de la pantalla. Nuevo `UndoToastContext.tsx`/hook `useUndoDelete()` (montado una sola vez en `App.tsx` vía `UndoToastProvider`) centraliza el temporizador del borrado pendiente y el estado del toast para que los 6 tabs compartan una sola implementación en vez de duplicarla; nuevo `UndoToast.tsx` renderiza el toast en sí. El `handleDelete` de cada tab ahora saca el ítem del estado local de forma optimista (en su posición original del array, para restaurarlo en el lugar correcto) y llama a `scheduleDelete(mensaje, execute, undo)` en vez de llamar directo al servicio de borrado. Si se programa un segundo borrado mientras el anterior sigue pendiente, el primero se confirma de inmediato en vez de apilar toasts. Verificado manualmente en el navegador de punta a punta (checklist): el ítem desaparece al borrar, aparece el toast, "Deshacer" lo restaura sin llamar al backend, y dejar que el temporizador expire seguido de una recarga de página confirma que el borrado real quedó persistido. Enteramente frontend — sin cambios en el backend ni en la base de datos, sin dependencias nuevas. |
| 2026-08-14 | feature/testing-foundation | Se estableció el patrón de testing de punta a punta — el repo no tenía ni un solo test antes de esto. Backend: nuevo proyecto xUnit `Bitacora.Infrastructure.Tests` (el roadmap originalmente lo llamaba `Bitacora.Application`, pero las implementaciones reales de los servicios viven en `Bitacora.Infrastructure`, que solo tiene DTOs/interfaces), testeando `ExpenseService` y `ExcelExportService` contra un `BitacoraDbContext` real respaldado por SQLite en memoria (`EnsureCreated()`, nunca `Migrate()`, ya que las migraciones existentes son específicas de Npgsql) en vez de mocks, porque los servicios reciben el `DbContext` concreto por constructor sin ninguna interfaz de repositorio que mockear — 27 tests, enfocados en la aislación de datos entre usuarios. Frontend: Vitest + React Testing Library (`vitest.config.ts` separado de `vite.config.ts` para que el plugin de PWA nunca corra durante los tests), cubriendo `alignEndDate` (100% de cobertura) y `ExpensesTab` (carga, estado vacío, la advertencia de configuración incompleta, barra de presupuesto, búsqueda/filtros, alta de un gasto, y el ciclo completo de undo-delete) — 22 tests. `dotnet test` y `npm run test` quedaron conectados al workflow de CI existente. También se resolvieron 6 hallazgos de severidad alta de `npm audit` (vía `npm audit fix`, sin bumps de versión rotos) y se fijó `SQLitePCLRaw.bundle_e_sqlite3` por encima de una vulnerabilidad conocida de SQLite. Se encontraron dos problemas que se dejaron sin arreglar a propósito para mantener esta rama enfocada solo en infraestructura, registrados como Paso 23 para una rama de fix aparte: `ExpensesTab` renderiza "Infinity%" cuando el presupuesto de un viaje es exactamente 0, y `Bitacora.API` carga una vulnerabilidad de gravedad alta conocida en `Microsoft.OpenApi 2.0.0`. Ninguna de las dos suites impone todavía un umbral de cobertura del 80% — la cobertura crece de forma incremental a medida que futuras features suman sus propios tests. |
